//Author: Jacob Liddicoat
//Date created: Mar 17,2018
public class ascii-art-assignment
 {
  public static void main(String args[])
  {
   String JJJJJ ="JJJJJ";
   String J ="J";
   String A ="A";
   String AAAAA ="AAAAA";
   String CCCC ="CCCC";
   String C ="C";
   String OOOO ="OOOO";
   String O ="O";
   String BBBB ="BBBB";
   String B ="B";

   System.out.print(JJJJJ + "   " + A); //Prints JJJJJ"   "A
   System.out.print("   " + CCCC + " "); //Prints "   "CCCC" "
   System.out.println(OOOO + " " + BBBB); //Prints OOOO" "BBBB

   System.out.print("  " + J + "    "); //Prints "  "J"    "
   System.out.print(A + " " + A + "  "); //Prints A" "A"  "
   System.out.print(C + "    " + O + "  " + O); //Prints C"    "O"  "O
   System.out.println(" " + B + "   " + B); //Prints " "B"   "B

   System.out.print("  " + J + "   "); //Prints "  "J"   "
   System.out.print(A + "   " + A + " "); //Prints A"   "A" "
   System.out.print(C + "    " + O + "  " + O); //Prints C"    "O"  "O
   System.out.println(" " + B + "   " + B); //Prints " "B"   "B

   System.out.print("  " + J + "   " + AAAAA + " "); //Prints "  "J"   "AAAAA" "
   System.out.println(C + "    " + O + "  " + O + " " + B + "   " + B); //Prints C"    "O"  "O" "B"   "B

   System.out.print("  " + J + "   " + A + "   " + A + " "); //Prints  "  "J"   "A"   "A" "
   System.out.println(C + "    " + O + "  " + O + " " + B + "   " + B); //Prints C"    "O"  "O" "B"   "B

   System.out.print(J + " " + J + "   " + A + "   " + A + " "); //Prints J" "J"   "A"   "A" "
   System.out.println(C + "    " + O + "  " + O + " " + B + "   " + B); // Prints C"   "O"  "O" "B"   "B

   System.out.print(" " + J + "    " + A + "   " + A + " "); //Prints " "J"    "A"   "A" "
   System.out.println(CCCC + " " + OOOO + " " + BBBB); //Prints CCCC" "OOOO" "BBBB
  }
 }
